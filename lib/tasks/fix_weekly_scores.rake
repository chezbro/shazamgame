namespace :fix do
  desc "Fix weekly scores on Heroku - set last_week_score to current weekly_points"
  task weekly_scores: :environment do
    ...
    puts "STEP 2: FIXING WEEKLY SCORES"
    puts "-" * 40
    ...
    User.find_each do |user|
      ...
      # Set last_week_score to current weekly_points (this represents the prior week's scores)
      user.last_week_score = user.weekly_points
      
      if user.save!
        users_updated += 1
        puts "✅ Updated #{user.username}:"
        puts "   last_week_score: #{old_last_week_score} → #{user.last_week_score}"
    ...
    if users_updated == total_users
      ...
      puts "✅ The 'Top Scores for the Week' leaderboard will now show prior week scores."
  ...
  task :check_weekly_scores => :environment do
    ...
    User.find_each do |user|
      if user.last_week_score == 0
        ...
        puts "⚠️  #{user.username}: last_week_score is 0 (needs fixing)"
      elsif user.last_week_score == user.weekly_points
        users_with_matching_scores += 1
        puts "✅ #{user.username}: scores match (last_week=#{user.last_week_score}, weekly=#{user.weekly_points})"
      else
        puts "ℹ️  #{user.username}: last_week=#{user.last_week_score}, weekly=#{user.weekly_points}"
      end
    end
    ...
    puts "Users with matching scores: #{users_with_matching_scores}"
    
    if users_with_zero_last_week > 0
      puts "⚠️  #{users_with_zero_last_week} users need their weekly scores fixed."
      puts "   Run 'rake fix:weekly_scores' to fix them."
    else
